...
public static class UtilityScorers
{
    ...
    {
        var enemies = UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.RelevantEnemyContacts, 0);
        var hasImmediate = UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.HasImmediateThreat, false);
        var band = (TacticalDistanceBand)UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.BestAttackTargetBand, (int)TacticalDistanceBand.OutOfRange);
        var hull = UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.OwnHullFraction, 1f);
        var threat = UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.LocalThreatScore, 0f);
    ...
    {
        if (!UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.CooldownReady, false)) return 0.04f;
        var enemy = UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.BestAttackTargetId, -1);
        if (enemy <= 0) enemy = UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.FocusTargetId, -1);
        if (enemy <= 0) return 0f;
        var band = (TacticalDistanceBand)UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.BestAttackTargetBand, (int)TacticalDistanceBand.OutOfRange);
        if (band == TacticalDistanceBand.OutOfRange) return 0.02f;
        var targetPriority = UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.BestAttackPriorityScore, 0f);
        var hull = UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.OwnHullFraction, 1f);
    ...
    {
        if (UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.IsDrone, false))
        {
            var droneHull = UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.OwnHullFraction, 1f);
            var droneThreat = UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.LocalThreatScore, 0f);
    ...
    {
        if (!UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.IsRepairShip, false)) return 0f;
        var ally = UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.BestRepairTargetId, -1);
        if (ally <= 0) return 0.03f;
        var support = UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.LocalSupportScore, 0f);
    ...
    public static readonly Consideration ScreenHighValue = new((_, a) =>
    {
        var support = UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.LocalSupportScore, 0f);
        var hasImmediate = UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.HasImmediateThreat, false);
        var baseline = UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.IsCommander, false) ? 0.24f : 0.15f;
    ...
    {
        if (!UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.IsCarrier, false)) return 0f;
        if (!UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.CooldownReady, false)) return 0.04f;
        var enemy = UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.BestAttackTargetId, -1);
    ...
    {
        if (!UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.IsCollective, false)) return 0f;
        if (UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.IsDrone, false)) return 0f;
        var hull = UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.OwnHullFraction, 1f);
    ...
    {
        var hasImmediate = UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.HasImmediateThreat, false);
        var allies = UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.RelevantAllyContacts, 0);
        var support = UtilityDiagnostics.Read(a, BenchmarkBlackboardKeys.LocalSupportScore, 0f);
