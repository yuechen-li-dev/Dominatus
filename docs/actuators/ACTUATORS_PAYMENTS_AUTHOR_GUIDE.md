...
| Provider adapter packages | Translation between Dominatus commands and one provider API, provider options, provider SDK dependency, provider idempotency mapping, provider-specific money conversion, explicit platform-fee mapping when supported and configured, provider status/error normalization, webhook verification where the adapter supports it, optional gated live smoke tests. | Kernel payment semantics, application identity models, fee policy, default fees, hidden runtime conditions, cross-provider routing policy, end-user UX, dashboards, business reactions to payment events. |
...
| `Dominatus.Actuators.Payments.PayPal` | Current PayPal Orders adapter. Translates supported Dominatus payment commands to PayPal Orders APIs where semantics match and clearly fails unsupported features. | PayPal SDK/API dependencies belong only in this adapter package. |
