...
public sealed class DeterministicTransitionTests
{
    ...
    public void CollectionExpressionAuthoring_CompilesAndDispatchesLifecycle()
    {
        ...
        var stopBeforeStart = definition.Dispatch(new NotStarted(), new StopRequested(), new Context());
        Assert.Equal(TransitionDispatchStatus.Rejected, stopBeforeStart.Inspection.Status);
        Assert.Null(stopBeforeStart.NextState);
    ...
    [Fact]
    public void Validation_ReportsStableRuleDiagnosticsAndDefinitionRefusesInvalidInput()
    {
        var flow = Transition.For<SessionState, SessionEvent, Context, Effect>();
        var duplicate = flow.On<NotStarted, StartRequested>("duplicate", static (_, _, _) => new(new Running()));
        var report = flow.Validate(
        [
            flow.On<NotStarted, StartRequested>(" ", static (_, _, _) => new(new Running())),
            duplicate,
            duplicate,
            flow.On<SessionState, SessionEvent>("base", static (_, _, _) => new(new Running())),
            flow.On<SessionState, SessionEvent>("shadow", static (_, _, _) => new(new Running())),
        ...
        Assert.Contains(report.Diagnostics, static d => d.Code == TransitionValidationCode.DuplicateRuleId && d.RuleIndex == 2 && d.RelatedRuleIndex == 1);
        Assert.Contains(report.Diagnostics, static d => d.Code == TransitionValidationCode.DuplicateUnguardedCase && d.RuleIndex == 2);
        Assert.Contains(report.Diagnostics, static d => d.Code == TransitionValidationCode.ShadowedRule && d.RuleIndex == 4);
