...
public sealed class AnthropicMessagesDecisionClientTests
{
    private static readonly Uri Endpoint = new("https://api.anthropic.com/v1/messages");

    [Fact]
    public async Task AnthropicDecisionClient_SendsXApiKeyHeader()
    {
    ...
    [Fact]
    public async Task AnthropicDecisionClient_IncludesDecisionPromptInMessage()
    {
        var transport = CreateTransport();
        var client = CreateClient(transport);
        var request = CreateRequest();
        ...
        var prompt = doc.RootElement.GetProperty("messages")[0].GetProperty("content").GetString();
        Assert.Contains("stableId: story.guard.approach.v1", prompt, StringComparison.Ordinal);
        Assert.Contains("- id: attack", prompt, StringComparison.Ordinal);
    }

    [Fact]
    public async Task AnthropicDecisionClient_ParsesRawJsonTextBlock()
    {
        var transport = CreateTransport(responseBody: AnthropicResponse(ValidDecisionJson));
        var client = CreateClient(transport);
        var request = CreateRequest();
    ...
    [Fact]
    public async Task AnthropicDecisionClient_RejectsMalformedDecisionJson()
    {
        var transport = CreateTransport(responseBody: AnthropicResponse("{\"scores\":[}]"));
        var client = CreateClient(transport);
        var request = CreateRequest();
    ...
    [Fact]
    public async Task AnthropicDecisionClient_DoesNotLeakApiKeyInErrors()
    {
        var transport = CreateTransport(statusCode: 401, responseBody: """
            {"error":{"type":"invalid_request_error","message":"bad test-anthropic-key-not-secret"}}
            """);
        var client = CreateClient(transport);
    ...
    [Fact]
    public async Task AnthropicDecisionClient_HonorsPreCanceledToken()
    {
        var transport = CreateTransport();
        var client = CreateClient(transport);
    ...
    [Fact]
    public void DecisionReplayMode_WithRecordedAnthropicDecisionResult_DoesNotCallTransport()
    {
        var request = CreateRequest();
    ...
    [Fact]
    public void DecisionStrictMode_WithRecordedAnthropicDecisionResult_DoesNotCallTransport()
    {
        var request = CreateRequest();
        var hash = LlmDecisionRequestHasher.ComputeHash(request);
        var cassette = new InMemoryLlmDecisionCassette();
    ...
    private static AnthropicMessagesDecisionClient CreateClient(FakeLlmHttpTransport transport)
        => new(new LlmHttpProviderOptions("anthropic", "claude-sonnet-4-20250514", Endpoint, "test-anthropic-key-not-secret"), transport);

    private static FakeLlmHttpTransport CreateTransport(int statusCode = 200, string? responseBody = null)
        => new(_ => new LlmHttpResponse(statusCode, responseBody ?? AnthropicResponse(ValidDecisionJson)));
    ...
    private static LlmDecisionRequest CreateRequest() => new(
        ...
        Sampling: new LlmSamplingOptions("anthropic", "claude-sonnet-4-20250514", Temperature: 0.0, TopP: 1.0, MaxOutputTokens: 256),
