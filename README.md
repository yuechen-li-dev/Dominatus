...
- [`samples/Dominatus.RTSBenchmark`](samples/Dominatus.RTSBenchmark) — runnable headless RTS-like CPU benchmark for deterministic agent-orchestration throughput with phase timing, hotspot diagnostics, and M3 tactical threat/support banding: ships make utility decisions, exchange events, and emit actions over thousands of ticks without LLMs, network, rendering, or GPU work. Run/docs: [RTS benchmark sample](docs/samples/SAMPLE_RTS_BENCHMARK.md).
