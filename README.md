...
- [`samples/Dominatus.RTSBenchmark`](samples/Dominatus.RTSBenchmark) — planned headless RTS-like CPU benchmark for deterministic agent-orchestration throughput: ships make utility decisions, exchange events, and emit actions over thousands of ticks without LLMs, network, rendering, or GPU work. Design contract: [RTS benchmark sample](docs/samples/SAMPLE_RTS_BENCHMARK.md).
